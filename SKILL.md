...
Infer as much as possible before asking. You usually need: the **topic** (and any must-have subtopics), the **time window** (default: last 3 months; "latest" means newest-first), **how many papers**, and the **depth tier** (below). Only ask if a choice genuinely changes the output and you can't infer it.

**Depth tiers** — pick one (default: Standard), and say which you're using:

- **Scan** — breadth over depth. Triage many papers; output is the MOC note only, one-line verdict + relevance per paper, no full notes. Good for "what's new in X this month".
- **Standard** — the default. A full note per selected paper applying all four insight layers (step 4). Good for 3–5 papers worth real attention.
- **Deep** — Standard plus the two adversarial tools in the rubric (inversion pre-mortem + first-principles strip). Reserve for the 1–2 papers that actually matter; it's slow, so don't apply it to a whole scan.

If the user dropped a specific paper/URL, default to Standard (or Deep if they signal it matters) and skip straight to step 4 for that paper.
...
1. **Methodology critique** — what's actually solid vs. shaky (eval gaps, missing baselines, weak stats, tiny n, cherry-picked benchmarks, no code/reproducibility). Cite the specific section/figure so the critique is checkable, and separate what the authors *claim* from what they actually *show*.
...
4. **What to read next + one action** — the 1–2 references or follow-ons that matter most, plus one concrete next action the user could take (e.g. clone the repo and run X, try the method on their dataset, email the author about Y).

On the **Deep tier**, also apply the two adversarial tools from the rubric — the inversion pre-mortem (how would this result fail to replicate?) and the first-principles strip (what's the real contribution once the framing is removed?). These are what separate a sharp note from a competent one.
...
- The critique separates the authors' **claims** from their **evidence**, and points at a specific section/figure.
- There's **one concrete next action** the user could take, not just "read more".
